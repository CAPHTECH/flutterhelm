...
Phase 5 implementation checks は repo root の `mise.toml` と Dart/Flutter package を使うため、先に toolchain を有効化します。
...
`runtime`, `profiling`, `bridge`, `interaction` は macOS + Xcode simulator 前提のローカル専用チェックです。
