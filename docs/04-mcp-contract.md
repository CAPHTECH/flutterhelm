...
- `experimental.adapterRegistry.families = ["delegate", "flutterCli", "profiling", "runtimeDriver", "platformBridge", "nativeBuild"]`
...
Sprint 16 では iOS-first の native build orchestration を beta として追加しました。  
...
## 4.13 Sprint 17: official delegate first

Sprint 17 では built-in `delegate` family の primary backend を official Flutter MCP (`dart mcp-server`) に切り替えました。

- provider id は `builtin.delegate.workspace` のまま維持
- `analyze_project`, `resolve_symbol`, `pub_search` は official-first
- `dependency_add`, `dependency_remove` は approval replay を維持したまま official-first
- `get_runtime_errors`, `get_widget_tree`, `hot_reload`, `hot_restart` は per-call DTD connect を行い official-first
- official delegate unavailable / timeout / malformed payload / DTD connect failure の場合は current backend に fallback
- public tool 名と resource URI は変えない
