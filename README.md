...
- Repo-local Codex plugin: [plugins/flutterhelm](plugins/flutterhelm)
  - default plugin path starts only FlutterHelm
  - `mobile-mcp` and native-build companion MCPs are opt-in and documented in plugin skills
...
- Always:
  ...
  - MCP client that supports Tools and Resources
- iOS workflows:
  - Xcode
  - `xcrun simctl`
- Beta `runtime_interaction`:
  - Node.js / `npx`
...
Sprint 16 では native build orchestration の beta wave を追加しました。`native_build` workflow、`nativeBuild` adapter family、`native-build` harness lane は iOS-first の beta として利用できますが、stable lane には入りません。
Sprint 17 では built-in delegate を official Flutter MCP first に切り替え、`analyze_project`、`resolve_symbol`、`pub_search`、dependency mutation、runtime diagnostics、hot reload/restart を official-first + automatic fallback で扱うようにしました。
delegate lane 単体の検証は `mise exec -- pnpm -C harness delegate` で実行できます。
...
公開 repo には repo-local Codex plugin も含まれています。plugin の default `.mcp.json` は stable の FlutterHelm だけを起動し、beta companion MCP は plugin skill の reference から opt-in で有効化する前提です。
